count_taxa <-
  function(checklist_dat) {
    ...
    checklist_dat3 <- checklist_dat[order(
      checklist_dat$GROUP,
      checklist_dat$FAMILY_NUMBER,
    ...
    no_genera_by_family_dat <-
      unique(data.frame(GENUS = checklist_dat3$GENUS, FAMILY = checklist_dat3$FAMILY))
    no_genera_by_family <-
      aggregate(
        no_genera_by_family_dat$GENUS,
    ...
    no_species_by_family_dat <-
      unique(data.frame(SPECIES = checklist_dat3$SPECIES, FAMILY = checklist_dat3$FAMILY))
    ...
    no_genera_by_group_dat <-
      unique(data.frame(GENUS = checklist_dat3$GENUS, GROUP = checklist_dat3$GROUP))
    no_genera_by_group <-
      aggregate(
        no_genera_by_group_dat$GENUS,
    ...
    no_species_by_group_dat <-
      unique(data.frame(SPECIES = checklist_dat3$SPECIES, GROUP = checklist_dat3$GROUP))
    no_species_by_group <-
      aggregate(
        no_species_by_group_dat$SPECIES,
    ...
    no_families_by_group_dat <-
      unique(data.frame(FAMILY = checklist_dat3$FAMILY, GROUP = checklist_dat3$GROUP))
    ...
    no_species_by_genus_dat <-
      unique(data.frame(SPECIES = checklist_dat3$SPECIES, GENUS = checklist_dat3$GENUS))
    no_species_by_genus <-
      aggregate(
        no_species_by_genus_dat$SPECIES,
