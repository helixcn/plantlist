...
function( checklist_dat = NULL,
    ...
    checklist_dat2 <- merge(checklist_dat, parsed_taxa_df, 
                            by.x = "SPECIES_FULL", 
                            by.y = "TAXON_PARSED")

    # reordering
    checklist_dat3 <- checklist_dat2[order(checklist_dat2$GROUP,
    ...
    for(m in 1:length(group_list)){
        ...
        temp_group_dat <- subset(checklist_dat3, checklist_dat3$GROUP == group_list[m])

        family_list <- unique(temp_group_dat$FAMILY)
        if(theme == "complex"){

            for(i in 1:length(family_list)){
                temp_family_dat <- subset(temp_group_dat, temp_group_dat$FAMILY == family_list[i])
                # Family Title
                main_text <- c(main_text,"", paste(unique(paste("## ", 
                                                                paste(as.numeric(temp_family_dat$FAMILY_NUMBER), 
                                                                      temp_family_dat$FAMILY_CN, 
                                                                      temp_family_dat$FAMILY), sep = "" 
                ) 
                ), collapse = "")
                )
                    ...
                    for(k in 1:length(temp_species_list)){
                        ...
                        main_text <- c(main_text, "", 
                                       c(# Species Chinese Name
                                           ...
                                           #}, 
                                           "",
                                           paste("**Specimens:**", ifelse(unique(temp_species_dat$SPECIMENS) == "", "", paste(unique(temp_species_dat$SPECIMENS))), collapse = ""),
                                           "",
                                           paste("**IUCN status:**", ifelse(unique(temp_species_dat$IUCN_CHINA) == "", "", paste(unique(temp_species_dat$IUCN_CHINA))), collapse = ""),
                                           "",
                                           paste("**Habitat:**", ifelse (unique(temp_species_dat$HABITAT) == "", "", paste(unique(temp_species_dat$HABITAT))), collapse = ""),
                                           "",
                                           paste("**Altitude:**", ifelse(unique(temp_species_dat$ALTITUDE) == "", "", paste(unique(temp_species_dat$ALTITUDE))), collapse = ""),
                                           "",
                                           paste("**Distribution:**", ifelse(unique(temp_species_dat$PROVINTIAL_DISTRIBUTION) == "", "", paste(unique(temp_species_dat$PROVINTIAL_DISTRIBUTION))), collapse = ""),
                                           " ",
                                           " ",
                                           " "
                                       ))
                    ...
                    for(k in 1:length(temp_species_list)){
                        ...
                        main_text <- c(main_text, "", 
                                       c(# Species Chinese Name
                                          ...
                                          # }, 
                                          #  "",
                                           paste("**Specimens:**", ifelse(any(is.na(temp_species_dat$SPECIMENS))|any(temp_species_dat$SPECIMENS == ""), 
                                                 paste(na.omit(unique(temp_species_dat$SPECIMENS)), collapse = ","), 
                                                paste(unique(temp_species_dat$SPECIMENS), collapse = "")), collapse = ""),
                                           "", ""
                                       ))
                    }
